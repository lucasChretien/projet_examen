...
if(!empty($modif_pr_prix2) and !empty($modif_pr_prix1) and !empty($modif_contenu)) {
    //  UPDATE `produits` SET `pr_titre` = 'ssww', `pr_contenu` = 'alll' WHERE `produits`.`id` = ;
    $req = "UPDATE produits SET pr_prix2 = '$modif_pr_prix2', pr_prix1 = '$modif_pr_prix1', pr_contenu = '$modif_contenu' WHERE id = $id";
    $pdo->exec($req);
